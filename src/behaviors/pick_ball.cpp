...
// --- move to approach and taget logic ---
...
// may or may not need this, had a ball pose call back in OG node too
void PickBall::ballPoseCallback(geometry_msgs::msg::PoseStamped::UniquePtr msg)
{
    ball_pose_ = std::make_shared<geometry_msgs::msg::PoseStamped>(*msg);

    // Retrieve the target pose from blackboard
    auto target_pose_expected = getInput<geometry_msgs::msg::PoseStamped::SharedPtr>("ball_pose");
    ...
    // in original node, this calculates transform from map -> body and rotates pose
}

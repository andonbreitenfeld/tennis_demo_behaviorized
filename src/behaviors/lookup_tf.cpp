...
LookupTF::LookupTF(const std::string &name, const BT::NodeConfig &config) : StatefulActionNode(name, config), Node(name)
{
  tf_buffer_ = std::make_unique<tf2_ros::Buffer>(this->get_clock());
  tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_);
}

BT::PortsList LookupTF::providedPorts() { return {BT::InputPort<std::string>("ref_frame"), BT::InputPort<std::string>("target_frame"), BT::InputPort<double>("timeout_secs"), BT::OutputPort<std::shared_ptr<geometry_msgs::msg::PoseStamped>>("pose")}; }

BT::NodeStatus LookupTF::onStart()
{
    if (!getInput<std::string>("ref_frame", ref_frame_))
    {
        throw BT::RuntimeError("missing required input [ref_frame]");
    }

    if (!getInput<std::string>("target_frame", target_frame_))
    {
        throw BT::RuntimeError("missing required input [target_frame]");
    }

    if (!getInput<double>("timeout_secs", timeout_secs_))
    {
        throw BT::RuntimeError("missing required input [timeout_secs]");
    }
...
BT::NodeStatus LookupTF::onRunning()
{
      ...
      try
      {
        ...
        tf_stamped = tf_buffer_->lookupTransform(
            ...
            tf2::durationFromSec(tf_lookup_timeout_secs_));  // short internal timeout
	...
	auto pose = std::make_shared<geometry_msgs::msg::PoseStamped>(pose_out);
      ...
      {
        // Not found yet — try again on next tick
        return BT::NodeStatus::RUNNING;
      }
}
